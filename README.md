...
4. **Mock de integración externa:** Si el SRI no responde, usar un JSON estático con datos de prueba (ver Anexo A).
...
Este anexo proporciona una simulación de la API del SRI para probar el sistema sin necesidad de conectarse al entorno real.

## A.1 Endpoints de la Fake API

| Método | Endpoint | Descripción |
|--------|----------|-------------|
| GET | `/api/sri/nota/{numeroAutorizacion}` | Obtener datos de una nota de crédito |
| GET | `/api/sri/validar/{numeroAutorizacion}` | Validar estado y saldo de una nota |
| GET | `/api/sri/ruc/{ruc}` | Consultar estado de un contribuyente |

## A.2 Estructura de Respuesta
...
## A.4 Documentos de Prueba

### XML de Nota de Crédito Válida
...
### XML de Nota de Crédito con Saldo Insuficiente
...
### XML de Nota de Crédito con Cadena de Endosos Rota
...
### Resumen de Documentos de Prueba

| Archivo | Situación | Resultado Esperado |
|---------|-----------|-------------------|
| `nota_valida.xml` | Flujo feliz completo | Extracción exitosa, sin riesgos, transición a LISTO_PARA_NEGOCIAR |
| `nota_saldo_insuficiente.xml` | Saldo 500, se solicita 1000 | Riesgo ALTO, bloqueo de transición |
| `nota_endosos_rotos.xml` | Cadena de endosos inconsistente | Riesgo CRÍTICO (MSG-15), bloqueo automático |
| `nota_bloqueada.xml` | Estado SRI = BLOQUEADA | Riesgo CRÍTICO, no permite negociación |
